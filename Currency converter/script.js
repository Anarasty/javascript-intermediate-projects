const startConverter = () => {
  ...
  const setCurrencies = () => {
    fromName.innerHTML =
      fromCurrency.options[fromCurrency.selectedIndex].dataset.name;
    toName.innerHTML =
      toCurrency.options[toCurrency.selectedIndex].dataset.name;
  };

  const convert = (value) => {
    errorMessage.style.display = "none";
    fetch(
      `https://free.currconv.com/api/v7/convert?apiKey=d4cf3228112bfb5a29f5&q=${fromCurrency.value}_${toCurrency.value}&compact=y`
    )
      .then((response) => response.json())
      .then((rate) => {
        toValue.value = (
          value * rate[`${fromCurrency.value}_${toCurrency.value}`].val
        ).toFixed(2);
      })
    ...
    .then(({ results }) => {
      ...
      for (const cur in ordered) {
        if (results.hasOwnProperty(cur)) {
          ...
          option.innerHTML = results[cur].id;
          frag.append(option);
        }
      }
